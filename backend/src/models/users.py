...
def get_user_by_id(db: psycopg.Connection, user_id: UUID) -> User | None:
    with db.cursor() as cursor:
        ...
        return User(user_id=row[0], name=str(row[1]), email=str(row[2]), gender=Gender(str(row[3])), description=str(row[4]))


@db_named_query
def get_user_by_email(db: psycopg.Connection, email: str) -> User | None:
    with db.cursor() as cursor:
        cursor.execute("SELECT id, name, email, gender, description FROM users WHERE email = %s", [str(email)])
        db.commit()
...
def create_user(db: psycopg.Connection, name: str, email: str, gender: Gender) -> User:
    ...
    with db.cursor() as cursor:
        cursor.execute(
            ...
            (str(user.user_id), str(user.name), str(user.email), str(user.gender), str(user.description)),
        )
        db.commit()
